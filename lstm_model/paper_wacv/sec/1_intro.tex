...
transfer on every script observed (up to $3.9\times$); % TODO(bbpe): 9/9 range at completion
...
\begin{enumerate}
  ...
  \item \textbf{The pivot is necessary, not incidental.} A preregistered stock-BPE ablation under
  identical data, seed, and recipe degrades WRR on every script (up to $3.9\times$), isolating
  \emph{output-space structure} --- not synthetic fine-tuning per se --- as the transfer
  mechanism. % TODO(bbpe): final wording at 9/9
